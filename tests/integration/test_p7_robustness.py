...
from volfoundry.pricers.monte_carlo import MCResult, mc_price, _generate_paths
...
class TestIvEdgeCases:
    ...
    def test_vectorized_recovers_vols(self):
        ...
        from volfoundry.iv.black_scholes import black76_price
        prices = np.array([
            black76_price(float(F_arr[i]), float(K_arr[i]), sigma_true,
                          float(T_arr[i]), float(r_arr[i]), CALL)
            for i in range(n)
        ])
...
class TestSviDiagnostics:
    ...
    def test_min_data_requirement(self):
        ...
        with pytest.raises(ValueError, match="at least 4"):
            calibrate_svi_slice(np.array([0.0, 0.1, 0.2]),
                              np.array([0.01, 0.02, 0.03]), 0.25)
...
class TestMcStructuredResults:
    ...
    def test_mc_result_is_structured(self):
        ...
        result = mc_price(100.0, 100.0, 0.20, 0.25, 0.05, CALL,
    ...
    def test_control_variate_near_bs(self):
        ...
        result = mc_price(F, K, sigma, T, r, CALL,
    ...
    def test_zero_vol_returns_exact_intrinsic(self):
        ...
        result = mc_price(100.0, 90.0, 0.0, 0.25, 0.05, CALL,
                          n_paths=1000, seed=1)
